...
def load_model(model_dir, ckpt_name, device):
    ...
    if state_dim is None:
        # Older checkpoint saved before state_dim was recorded: infer it
        # from the trained weights themselves (output layer's out_features).
        state_dict = ckpt["model_state_dict"]
        last_weight_key = [k for k in state_dict.keys() if k.endswith(".weight")][-1]
        state_dim = state_dict[last_weight_key].shape[0]
        print(f"WARNING: 'state_dim' not found in checkpoint, inferred from weights: {state_dim}")
...
def compute_flat_metrics(pred_states, target_states):
    # States are built as concatenate([positions, velocities], axis=1) with
    # equal-sized halves, so split at the midpoint rather than assuming a
    # fixed 2D position / 2D velocity layout. This generalizes to any number
    # of balls/objects (e.g. 4+4 for single ball, 4+4 for billiards with 2 balls, etc.)
    d = pred_states.shape[1]
    half = d // 2

    pred_pos = pred_states[:, :half]
    target_pos = target_states[:, :half]

    pred_vel = pred_states[:, half:]
    target_vel = target_states[:, half:]
...
def compute_rollout_metrics(pred_seq, target_seq):
    ...
    for step in range(h):
        ...
        pred_pos = pred_step[:, :half]
        target_pos = target_step[:, :half]

        pred_vel = pred_step[:, half:]
        target_vel = target_step[:, half:]
...
def resolve_eval_dirs(args):
    ...
    if args.probe_train_dir is not None:
    ...
    elif args.legacy_split:
        # Reproduces the old behavior: treat data_dir as the same directory
        # used at training time and only evaluate on its held-out val slice.
        # Only meaningful if data_dir is the SAME directory used for training
        # (and with the same --val_ratio/--seed), e.g. re-checking val
        # performance on physics-data-billiard after training on it.
        ...
        # Default: evaluate on every trajectory in data_dir. Correct choice
        # for dedicated eval/OOD directories (e.g. physics-data-*-ood-*)
        # that aren't meant to be split further.
        probe_train_dirs = None
        split_mode = "full_directory"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--legacy_split",
        action="store_true",
        help="Apply the old 90/10 internal split to data_dir and evaluate only "
             "on the held-out val slice, instead of the full directory. Only "
             "meaningful when data_dir is the same directory used at training "
             "time (with matching --val_ratio/--seed). Ignored if "
             "--probe_train_dir is set."
    )
